...
@pytest.mark.asyncio
async def test_create_user(client, new_user):
    response = client.post('/v1/auth/register', content=json.dumps(new_user))
...
@pytest.mark.asyncio
async def test_set_default_user_group(client, new_user):
    response = client.post('/v1/auth/register', content=json.dumps(new_user))
...
@pytest.mark.asyncio
async def test_error_create_user_with_same_login(client, new_user):
    response = client.post('/v1/auth/register', content=json.dumps(new_user))
...
async def test_save_tokens_to_redis(client, redis):
    ...
    redis_data = await redis.get(new_user['login'])

    redis_tokens = json.loads(redis_data)


    assert redis_tokens['access_token'] == token

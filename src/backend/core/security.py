...
from src.backend.db.redis import repository
...
async def load_token_from_redis(data: dict, session: repository.Redis) -> dict:
    ...
    redis_data = await repository.get_item(login, session)
